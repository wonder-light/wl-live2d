...
export class UBaseStageController extends UBaseController {
   ...
   * @return {UBaseStageController} 自身引用
   */
  fadeIn(element = null, transitionend = null) {
   ...
   * @return {UBaseStageController} 自身引用
   */
  fadeOut(element = null, transitionend = null) {
    element ??= this.wrapper;
    element.ontransitionend = transitionend;
    // 添加过度类
    !element.classList.contains('live2d-transition-all') && element.classList.add('live2d-transition-all');
  ...
  _onModelLoad(style) {
    // 画布宽和高
    this.#data.canvas.style.width = this.#data.wrapper.style.width = `${ style.width }px`;
    this.#data.canvas.style.height = this.#data.wrapper.style.height = `${ style.height }px`;
    // 当包装器元素的宽度与高度被设置后，调整一次模型的大小
    this.options.app.resize();
    // 舞台淡入
    this.fadeIn(null, () => this.options.event.emit());
  }
}
