...
export class UStageController extends UBaseController {
  ...
  init() {
    ...
    const { minWidth, minHeight, offsetX, offsetY } = this.live2d.tips.data;
    // 添加类
    ...
    // 提示框
    this.tips.style.minWidth = `${ minWidth }px`;
    this.tips.style.minHeight = `${ minHeight }px`;
    this.tips.style.setProperty('--tips-offset-x', `${ offsetX }px`);
    this.tips.style.setProperty('--tips-offset-y', `${ offsetY }px`);
    // 绑定事件
