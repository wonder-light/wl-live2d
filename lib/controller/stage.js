...
export class UBaseStageController extends UBaseController {
  ...
  constructor(live2d, selector = null, data = null) {
    ...
    this.event.on(EEvent.modelError, this.fadeOut.bind(this, this.wrapper), this);
  }
  ...
  async fadeIn(element = null) {
    await this._fade(element, 'fadeIn', 'fadeOut').catch(() => {});
  }
  ...
  async fadeOut(element = null) {
    await this._fade(element, 'fadeOut', 'fadeIn').catch(() => {});
  }
  ...
  async _fade(element = null, proceed, exit) {
    ...
    let time = this._getTransitionDuration(element);
    // 添加过度类
  ...
  _getTransitionDuration(element) {
    if (!element) return 0;
    const str = getComputedStyle(element).getPropertyValue('transition-duration');
    return FHelp.defaultTo(0, parseFloat(str)) * (str.length === 1 ? 1000 : 1);
  }
}
