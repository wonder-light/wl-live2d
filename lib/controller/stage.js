...
export class UBaseStageController extends UBaseController {
  ...
  destroy() {
    ...
    const ref = this.options.ref['_showAndHiddenMenus']
  ...
  fadeIn(element = null) {
    ...
    this._options.event.emit(EEvent.fadeStart);
    element.ontransitionend = () => {
      this._options.event.emit(EEvent.fadeEnd);
      // 清除
   ...
   * @return {Element | Node | null} 返回获取到的节点
   */
   ...
   */
  _bindEvent() {
    const ref = this.options.ref['_showAndHiddenMenus'] = this._showAndHiddenMenus.bind(this);
  ...
  _onModelLoad(style) {
    ...
    this.canvas.style.width = `${ style.width }px`;
    this.wrapper.style.width = `${ style.width }px`;
    this.canvas.style.height = `${ style.height }px`;
    this.wrapper.style.height = `${ style.height }px`;
