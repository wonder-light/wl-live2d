...
import { ULive2dController } from './live2d.js';
...
export class UBaseModelController extends UBaseController {
   ...
   */
  init() {
    this.loadModel(this.modelId, this.textureId);
    return this;
  }
  ...
  loadModel(modelId, textureId = 0) {
    ...
    const stage = this.live2d.app.stage;
    ...
    const model = this._model = FHelp.fromSync(url, {
      onError: (e) => {
  ...
  switchModel(id, textureId = 0) {
    ...
    const event = this.event;
    event.once(EEvent.fadeEnd, this.loadModel.bind(this, id, textureId));
    event.once(EEvent.modelLoad, stage.fadeIn.bind(stage, stage.canvas));
    // 此时 wrapper 不为 display:none , 可以获取宽高数据, canvas 渲染不会出错
    stage.fadeOut(stage.canvas);
    return this;
  }
  ...
  nextModel() {
    ...
    this.switchModel(this.modelId >= max ? 0 : this.modelId + 1, 0);
    return this;
  }
  ...
  nextTexture() {
    ...
    this.switchModel(this.modelId, this.textureId >= max ? 0 : this.textureId + 1);
    return this;
  }
