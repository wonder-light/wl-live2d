...
export class UBaseModelController {
   ...
   */
  async loadModel() {
    let current = this.#options.models[this.modelIndex];
    let url = current?.path ?? '';
    const model = this.#model = await Live2DModel.from(url, {
      onError: (e) => this.#model.emit('loadError', e)
    });
    // 设置模型数据
    if (current) {
      current?.width && (model.width = current.width);
      current?.height && (model.height = current.height);
      current?.position && (model.position = current.position);
      current?.scale && (model.scale = {x: current.scale, y: current.scale});
      current?.volume && (SoundManager.volume = current.volume);
    }
    this.#options.app.stage.addChildAt(model, 0);
