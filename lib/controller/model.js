...
export class UBaseModelController extends UBaseController {
  ...
  set textureId(index) {
    ...
    localStorage.setItem('texture-id', `${ this._modelId }`);
  }
  ...
  loadModel(modelId, textureId = 0) {
    ...
    const url = is(String, current?.path) ? current.path : is(Array, current?.path) ? current.path[textureId] : '';
    ...
    this._model.once('load', () => {
      ...
      this.options.app.stage.setChildIndex(model, 0);
  ...
  switchModel(id, textureId = 0) {
    ...
    stage.fadeOut(stage.canvas, () => this.loadModel.call(this, id, textureId));
    event.once(EEvent.modelLoad, () => {
      stage.fadeIn(stage.canvas);
    }, this);
    return this;
  }
  ...
  /**
   * 切换下一个模型
   * @return {UBaseModelController} 注释引用
   */
  nextModel() {
