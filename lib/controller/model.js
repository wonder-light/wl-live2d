...
import { clamp, defaultTo, is } from 'ramda';
...
export class UBaseModelController extends UBaseController {
   ...
   * @param {number} [textureId=0] 模型贴图ID
  ...
  motion() {
    ...
    this._model.internalModel.motionManager.on('motionStart', (group, index, audio) => {
      ...
      this.options.event.emit(EEvent.motionStart, group, index, audio);
    });
    this._model.internalModel.motionManager.on('motionFinish', () => {
      this._currentMotion = null;
      this.options.event.emit(EEvent.motionFinish);
    });
    return this;
  }
