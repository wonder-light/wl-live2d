import { groupBy, is, mergeAll } from 'ramda';
...
export class UBaseTipsController extends UBaseController {
  ...
  getRandomMessage() {
    ...
    let group = groupBy((m) => m.priority)(this._messages.filter(m => m.condition()));
    // 优先级小的在前面
    ...
    // 需要显示的消息
    let index = Math.floor(Math.random() * group[key].length);
    const target = group[key][index];
    if (is(Array, target.text)) {
      index = Math.floor(Math.random() * target.text.length);
      return target.text[index];
    }
    return target.text;
  }
