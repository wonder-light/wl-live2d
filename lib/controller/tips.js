...
export class UBaseTipsController extends UBaseController {
  ...
  fadeIn(inherit = false) {
    ...
    this.options.stage.fadeIn(this._tips, () => {
      // 淡入完成后计时
      if (!inherit) {
        clearTimeout(this._showId);
        this._showId = setTimeout(() => this.fadeOut.call(this), this.duration);
      }
    });
  }
  ...
  fadeOut(inherit) {
    this._showId = null;
    this.options.stage.fadeOut(this._tips, () => {
      // 淡出完成后计时
      if (!inherit) {
        clearTimeout(this._hiddenId);
        this._hiddenId = setTimeout(() => this.fadeIn.call(this), this.interval);
      }
    });
  }
