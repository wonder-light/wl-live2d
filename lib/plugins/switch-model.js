 ...
 */
export class FSwitchModulePlugin extends IBasePlugin {
  /**
   * 文档元素: 切换模型
   * @protected
   * @type {HTMLElement}
  ...
  install(options) {
    ...
    `;
    // 添加事件监听
    this._button.addEventListener('click', this.switchModule.bind(this));
    this._options.stage.addMenu(this._button, this._priority);
  }

  uninstall(options) {
    this._button.removeEventListener('click', this.switchModule);
