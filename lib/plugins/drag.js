...
export class FDragPlugin extends FBasePlugin {
  ...
  install(live2d) {
    ...
    this.#ref.start = this.dragLive2d.bind(this);
    this.#ref.run = this._run.bind(this);
    this.#ref.end = this._end.bind(this);
    this._wrapper.addEventListener('mousedown', this.#ref.start, { passive: true });
    this._wrapper.addEventListener('touchstart', this.#ref.start, { passive: true });
  }
