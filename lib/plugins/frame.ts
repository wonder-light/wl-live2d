...
export class FHitFramesPlugin extends FBasePlugin {
   ...
   * @private
   */
  private _functions?: (events: any) => void = undefined;
  ...
  public override install(live2d: ULive2dController): void {
    ...
    // 添加事件监听
    this._functions = this.clickTestFrame.bind(this);
    this._button.addEventListener('click', this._functions);
  ...
  public override uninstall(live2d: ULive2dController): void {
    if (!this._enable) return;
    this._button?.removeEventListener('click', this._functions!);
