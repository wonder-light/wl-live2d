...
export class FQuitPlugin extends FBasePlugin {
  ...
  install(options) {
    ...
    const { stage, data } = this._options;
    // 关闭按钮
    ...
    // 添加事件监听
    const ref1 = this._options.ref['hiddenLive2d'] = this.hiddenLive2d.bind(this);
    const ref2 = this._options.ref['showLive2d'] = this.showLive2d.bind(this);
  ...
  uninstall(options) {
    ...
    const ref1 = this._options.ref['hiddenLive2d'];
    const ref2 = this._options.ref['showLive2d'];
  ...
  showLive2d() {
    // 淡入包装器元素
    const { stage, model, event } = this._options;
    // wrapper 为 display:none 时, 没有宽高数据, canvas 渲染会出错
    stage.wrapper.classList.remove('live2d-hidden');
    event.once(EEvent.modelLoad, () => {
      stage.fadeIn(stage.wrapper);
      stage.fadeOut(this._show);
    });
    model.resetModel();
  }
