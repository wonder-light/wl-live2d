 ...
 */
export class FBaseSwitchPlugin extends IBasePlugin {
  /**
   * 文档元素: 切换按钮
   * @protected
   * @type {HTMLElement}
  ...
  install(options) {
    ...
    // 添加事件监听
    this._button.addEventListener('click', this.switch.bind(this));
    this._options.stage.addMenu(this._button, this._priority);
  }

  uninstall(options) {
    this._button.removeEventListener('click', this.switch);
