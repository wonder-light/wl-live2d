...
export class FHomePlugin extends FBasePlugin {
   ...
   * @protected
   */
  protected _back: HTMLDivElement | null = null;
  ...
  public override install(live2d: ULive2dController): void {
    ...
    `;
    // 添加事件监听
    const ref = this.live2d.ref['backToHome'] = this.backToHome.bind(this);
  ...
  public override uninstall(live2d: ULive2dController): void {
    ...
    location.pathname;
    const ref = this.live2d.ref['backToHome'];
  ...
  public backToHome(): void {
    const url = window.location.origin + this.live2d.data.homePath;
