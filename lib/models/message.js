/**
 * 事件消息的绑定与卸载函数的定义
 * @callback DBaseEventMessage~TEventMessageBind
 * @return {void}
 */
 ...
 * @alias FTimeMessage
 */
export class FTimeMessage extends DBaseMessage {
  /**
   * 指定的小时时间段,
   * 例: 2-4, 4,
   * 如果为 null 则为正常消息
  ...
  /**
   * 创建时间消息数据
   * @param {?module:modules.FTimeMessage} options 时间消息选项
   */
  constructor(options= null) {
    super(options);
...
export class DSeasonsMessage extends DBaseMessage {
   ...
   * 创建季节消息数据
   * @constructor
   * @param {?module:modules.DSeasonsMessage} options 季节消息数据
   */
  constructor(options = null) {
    super(options);
...
export class DBaseEventMessage extends DBaseMessage {
  ...
  /**
   * 丙丁指定事件
   * @type {?DBaseEventMessage~TEventMessageBind}
   * @default null
   */
  bind;
   ...
   * 创建事件消息数据
   * @constructor
   * @param {?module:modules.DBaseEventMessage} options 事件消息数据
   */
  constructor(options = null) {
    super(options);
